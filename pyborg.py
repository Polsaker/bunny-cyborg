...
class pyborg:
    ...
    def __init__(self):
        ...
        if self.settings.process_with == "pyborg":
            ...
            try:
                ...
                s = f.read()
                f.close()
                if s.decode() != self.saves_version:
    ...
    def save_all(self, restart_timer = True):
        if self.settings.process_with == "pyborg" and self.settings.no_save != "True":
            ...
            #print(wordlist)
            wordlist = sorted(wordlist, key=wordlist.index)
            ...
            wordlist = sorted(wordlist, key=lambda x,y: cmp(y[1],x[1]))
            list(map( (lambda x: f.write(str(x[0])+"\n") ), wordlist))
            f.close()
    ...
    def reply(self, body):
        try:
            ...
            while done == 0:
                ...
                liste = sorted(liste, key=liste.index)
